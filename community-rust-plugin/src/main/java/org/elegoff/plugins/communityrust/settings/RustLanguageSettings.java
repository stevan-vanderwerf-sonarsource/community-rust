...
public class RustLanguageSettings {
    ...
    private static final String CONFIG_FILE_PATH = "./extensions/plugins/config.properties"; // Relative path
    ...
    private static void loadProperties() {
        try (InputStream input = new FileInputStream(Paths.get(CONFIG_FILE_PATH).toAbsolutePath().toString())) {
            // Load the properties file
            properties.load(input);
        } catch (IOException ex) {
            ex.printStackTrace();
            // Handle the error appropriately (e.g., set default values or log an error)
        }
    }
    ...
    public static final String FILE_SUFFIXES_DEFAULT_VALUE = properties.getProperty(FILE_SUFFIXES_KEY, ".rs");
    ...
    public static List<PropertyDefinition> getProperties() {
        return asList(
            ...
            PropertyDefinition.builder(FILTER_UTF8_LB_KEY)
                ...
                .defaultValue(properties.getProperty(FILTER_UTF8_LB_KEY, "false"))
                .category("Rust")
                .onQualifiers(Qualifiers.PROJECT)
                .build());
    }
